...
function nodeId (parent) {
  if (parent) {
    return [parent.id, parent.children.length ].join('.');
  }
  else {
    return '0';
  }
...
Arboreal.parse = function (object, childrenAttr, onComplete) {
  var root,
...
Arboreal.prototype.find = function (id) {
  var match = null,
  ...
  this.traverseDown(iterator);
  return match;
};

Arboreal.prototype.path = function (path) {
